...
import { createKbService, createMemoryServiceForApp, loadKbConfigFromEnv, loadMemoryConfigFromEnv } from "./services/index.js";
import { setHeartbeatKbService, setHeartbeatMemoryService } from "./services/heartbeat-kb-hook.js";
...
export async function createApp(
  ...
  const memoryConfig = loadMemoryConfigFromEnv();
  const kbServiceInstance = createKbService(db, kbConfig);
  const memoryServiceInstance = createMemoryServiceForApp(db, memoryConfig);
  setHeartbeatKbService(kbServiceInstance);
  setHeartbeatMemoryService(memoryServiceInstance);
  api.use(kbRoutes(kbServiceInstance));
  api.use(memoryRoutes(memoryServiceInstance));
